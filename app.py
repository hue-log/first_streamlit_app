...
import pypdf  # replaces PyPDF2, more robust
...
if 'user_answers' not in st.session_state:
    st.session_state.user_answers = {}  # question index -> selected option index
...
def generate_mcq_from_chunk(client, chunk, n):
    """
...
if uploaded_file is not None and api_key:
    if st.button("🚀 Generate MCQs"):
        ...
        if not full_text.strip():
            ...
            if len(chunks) > 3:
                chunks = chunks[:3]   # limit API calls for very long PDFs
            all_mcqs = []
            ...
            with st.spinner("Generating questions with OpenAI (this may take a minute)..."):
                ...
                for chunk in chunks:
                    mcqs = generate_mcq_from_chunk(client, chunk, q_per_chunk)
                    all_mcqs.extend(mcqs)
                all_mcqs = all_mcqs[:num_questions]  # trim to exact desired number
            if all_mcqs:
...
if st.session_state.generated and st.session_state.quiz:
    ...
    for idx, q in enumerate(st.session_state.quiz):
        ...
        options = q["options"]
        # Radio returns the string, we later map it to its index
        choice = st.radio(
    ...
    if st.button("✅ Check Answers"):
        if len(st.session_state.user_answers) != len(st.session_state.quiz):
            ...
            for idx, q in enumerate(st.session_state.quiz):
                ...
                # Expandable feedback for each question
