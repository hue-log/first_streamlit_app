...
# ========== STREAMLIT APP ==========
st.set_page_config(page_title="CISA Domain 1 Practice", layout="wide")

# Session state
if "questions" not in st.session_state:
    st.session_state.questions = DOMAIN1_QUESTIONS.copy()
...
# Domain filter (only Domain 1 present, but we keep the UI for consistency)
domains = sorted({q["domain"] for q in DOMAIN1_QUESTIONS})
...
if st.sidebar.button("Apply Filter"):
    filtered = [q for q in DOMAIN1_QUESTIONS if q["domain"] in selected_domains]
...
if st.session_state.shuffled_data is None:
    ...
    # Build list of (option_text, is_correct) from the original order
    correct_idx = ord(correct_letter) - ord("A")
    opts_with_flag = []
    for i, opt in enumerate(q["options"]):
        opts_with_flag.append((opt, i == correct_idx))
    random.shuffle(opts_with_flag)
    st.session_state.shuffled_data = opts_with_flag

shuffled = st.session_state.shuffled_data
labels = ["A", "B", "C", "D"]
radio_choices = [f"{labels[i]}) {shuffled[i][0][3:].strip()}" for i in range(4)]  # Remove "X) " prefix for display
...
if not st.session_state.answered:
    ...
    for i, (orig_text, is_correct) in enumerate(shuffled):
        letter = labels[i]
        display_text = f"{letter}) {orig_text[3:].strip()}"  # remove original "X) "
        explanation = explanations.get(orig_text, "No explanation available.")
    ...
    if st.session_state.idx < total - 1:
        ...
        st.markdown("---")
        st.subheader("🎉 Domain 1 Complete!")
